      ...
      for (const row of rows) {
        await prisma.examSession.create({
          data: {
            ...
            totalTabSwitches: Number(row.totalTabSwitches ?? 0),
            totalTimeAway: Number(row.totalTimeAway ?? 0),
      ...
      for (const row of rows) {
        await prisma.answer.create({
          data: {
            ...
            tabSwitchTime: Number(row.tabSwitchTime ?? 0),
            ...
            tabSwitchTimeMs: Number(row.tabSwitchTimeMs ?? 0),
            createdAt: row.createdAt ? new Date(row.createdAt as string) : undefined,
            updatedAt: row.updatedAt ? new Date(row.updatedAt as string) : undefined,
          },
        });
      ...
      for (const row of rows) {
        await prisma.examLog.create({
          data: {
            ...
            durationAway: row.durationAway != null ? Number(row.durationAway) : undefined,
            metadata: (row.metadata as string) ?? undefined,
            examQuestionId: (row.examQuestionId as string) ?? undefined,
            createdAt: row.createdAt ? new Date(row.createdAt as string) : undefined,
          },
        });
      }
